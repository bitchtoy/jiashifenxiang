...
public class LeaseOrderAdapter extends BaseQuickAdapter<Integer, RecyclerViewHolder> {
    private int tag = 0;

    public LeaseOrderAdapter(@LayoutRes int layoutResId, @Nullable List<Integer> data, int tag) {
    ...
    protected void convert(RecyclerViewHolder helper, Integer item) {
        ...
        helper.addOnClickListener(R.id.tv_contact_owner);
        switch (tag) {
            case 0:
            ...
            case 1:
                helper.setVisible(R.id.tv_cancel,true);
                helper.setVisible(R.id.tv_contact_owner,true);
                break;
            case 2:
